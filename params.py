...
parser.add_argument(
    "--no_cuda", action="store_true", default=True, help="Enables CUDA training."
)
parser.add_argument(
    "--train", action="store_true", default=False, help="Train data."
)
parser.add_argument(
...
parser.add_argument(
    "--l2", type=float, default=0, help="Weight decay (L2 loss on parameters)."
)
parser.add_argument("--hop", type=int, default=1, help="Number of hops.")
...
parser.add_argument("--data", type=str, default="aifb", help="dataset.")

args = parser.parse_args()
args.name = (
    args.name + "_" + time.strftime("%d_%m_%Y") + "_" + time.strftime("%H:%M:%S")
)
args.using_cuda = not args.no_cuda
